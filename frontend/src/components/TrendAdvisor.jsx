...
function SummaryTable({title, rows, unit}){return <div className="card"><h2>{title}</h2><table><thead><tr><th>Rank</th><th>Combination</th><th>Total</th><th>Avg/day</th><th>Slope/day</th><th>Direction</th><th>R²</th><th>% Change</th></tr></thead><tbody>{rows.map((r,i)=><tr key={r.combo}><td>{i+1}</td><td><b>{r.combo}</b></td><td>{fmt(r.total,unit==='count'?0:1)}</td><td>{fmt(r.average_per_day,1)}</td><td>{fmt(r.slope_per_day,2)}</td><td><span className={'pill '+r.direction.toLowerCase()}>{r.direction}</span></td><td>{fmt(r.r2,2)}</td><td>{r.pct_change==null?'—':fmt(r.pct_change,1)+'%'}</td></tr>)}</tbody></table></div>}
